...
async def get_all_users(offset: int = 0, limit: int = 50) -> list[User]:
    ...
    Generates the event "READ" for the stream "user" sending as data the given
...
async def delete_user(primary_key: str) -> int:
    ...
    events confirm the deletion, then it will be performed. The
    deletion will take place once and upcoming event confirms it
    ...
    Returns:
        int:
            1: if the user was found and its deletion event is sent
    ...
    try:
        ...
        await asyncio.gather(
            UserDAL().get_user_by_id(primary_key),
            redis_stream.xadd('user', event)
        )
        return 1
    except NotFoundError:
